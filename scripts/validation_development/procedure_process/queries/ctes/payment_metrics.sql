...
PaymentMetrics AS (
    SELECT 
        ...
        COALESCE(SUM(
            ...
            THEN ps.SplitAmt ELSE 0 END
        ), 0) AS TotalPatientPaid,
        SUM(CASE WHEN DATEDIFF(CURRENT_DATE, bp.ProcDate) <= 30 THEN bp.ProcFee ELSE 0 END) AS Fees0to30Days,
        SUM(CASE WHEN DATEDIFF(CURRENT_DATE, bp.ProcDate) BETWEEN 31 AND 60 THEN bp.ProcFee ELSE 0 END) AS Fees31to60Days,
        SUM(CASE WHEN DATEDIFF(CURRENT_DATE, bp.ProcDate) BETWEEN 61 AND 90 THEN bp.ProcFee ELSE 0 END) AS Fees61to90Days,
        SUM(CASE WHEN DATEDIFF(CURRENT_DATE, bp.ProcDate) > 90 THEN bp.ProcFee ELSE 0 END) AS FeesOver90Days,
        COALESCE(SUM(
            CASE WHEN DATEDIFF(CURRENT_DATE, COALESCE(cp.ProcDate, ps.ProcDate)) <= 30
            THEN COALESCE(cp.InsPayAmt, 0) + COALESCE(ps.SplitAmt, 0) ELSE 0 END
        ), 0) AS Paid0to30Days,
        COALESCE(SUM(
        ...
        CASE 
            WHEN SUM(CASE WHEN DATEDIFF(CURRENT_DATE, bp.ProcDate) > 30 THEN bp.ProcFee ELSE 0 END) > 0 
            THEN ROUND(100.0 * (
                COALESCE(SUM(
                    CASE WHEN cp.ProcNum IS NOT NULL 
                         AND cp.InsPayAmt > 0 
            ...
            ) / SUM(CASE WHEN DATEDIFF(CURRENT_DATE, bp.ProcDate) > 30 THEN bp.ProcFee ELSE 0 END), 1)
            ELSE 0
