...
def export_data():
    """
    Main function to export data based on the 'patient_query'.
    """
    try:
        # Retrieve file path for patient_query export using the helper function
        output_path = get_file_path("patient_query")
        if not output_path:
            raise KeyError("File path for 'patient_query' is missing in file_paths.py")
