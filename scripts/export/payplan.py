...
def export_data():
    """
    Main function to export data based on the 'payplan_query'.
    """
    try:
        # Retrieve file path for payplan_query export using the helper function
        output_path = get_file_path("payplan_query")
        if not output_path:
            raise KeyError("File path for 'payplan_query' is missing in file_paths.py")
        ...
        query = get_sql_query("payplan_query")
