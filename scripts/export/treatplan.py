...
def export_data():
    """
    Main function to export data based on the 'treatplan_query'.
    """
    try:
        # Retrieve file path for treatplan_query export using the helper function
        output_path = get_file_path("treatplan_query")
        if not output_path:
            raise KeyError("File path for 'treatplan_query' is missing in file_paths.py")
        ...
        query = get_sql_query("treatplan_query")
