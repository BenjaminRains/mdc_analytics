...
def export_data():
    """
    Main function to export data based on the 'recall_query'.
    """
    try:
        # Retrieve file path for recall_query export using the helper function
        output_path = get_file_path("recall_query")
        if not output_path:
            raise KeyError("File path for 'recall_query' is missing in file_paths.py")
