...
def export_data():
    """
    Main function to export data based on the 'payment_query'.
    """
    try:
        # Retrieve file path for payment_query export using the helper function
        output_path = get_file_path("payment_query")
        if not output_path:
            raise KeyError("File path for 'payment_query' is missing in file_paths.py")
        ...
        query = get_sql_query("payment_query")
