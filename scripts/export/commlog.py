...
def export_data():
    """
    Main function to export data based on the 'commlog_query'.
    """
    try:
        # Retrieve file path for commlog_query export using the helper function
        output_path = get_file_path("commlog_query")
        if not output_path:
            raise KeyError("File path for 'commlog_query' is missing in file_paths.py")
