...
def export_data():
    """
    Main function to export data based on the 'procnote_query'.
    """
    try:
        # Retrieve file path for procnote_query export using the helper function
        output_path = get_file_path("procnote_query")
        if not output_path:
            raise KeyError("File path for 'procnote_query' is missing in file_paths.py")
        ...
        query = get_sql_query("procnote_query")
