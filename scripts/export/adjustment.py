...
def export_data():
    """
    Main function to export data based on the 'adjustment_query'.
    """
    try:
        # Retrieve file path for adjustment_query export using the helper function
        output_path = get_file_path("adjustment_query")
        if not output_path:
            raise KeyError("File path for 'adjustment_query' is missing in file_paths.py")
        ...
        query = get_sql_query("adjustment_query")
