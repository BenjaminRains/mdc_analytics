...
def export_data():
    """
    Main function to export data based on the 'proctp_query'.
    """
    try:
        # Retrieve file path for proctp_query export using the helper function
        output_path = get_file_path("proctp_query")
        if not output_path:
            raise KeyError("File path for 'proctp_query' is missing in file_paths.py")
        ...
        query = get_sql_query("proctp_query")
