...
def export_data():
    """
    Main function to export data based on the 'fee_query'.
    """
    try:
        # Retrieve file path for fee_query export using the helper function
        output_path = get_file_path("fee_query")
        if not output_path:
            raise KeyError("File path for 'fee_query' is missing in file_paths.py")
