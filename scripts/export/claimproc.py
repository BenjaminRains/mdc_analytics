...
def export_data():
    """
    Main function to export data based on the 'claimproc_query'.
    """
    try:
        # Retrieve file path for claimproc_query export using the helper function
        output_path = get_file_path("claimproc_query")
        if not output_path:
            raise KeyError("File path for 'claimproc_query' is missing in file_paths.py")
        ...
        query = get_sql_query("claimproc_query")
